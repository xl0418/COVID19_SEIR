corona_mapper <- function(date = '2020-04-21', mode = 'Province/State'){
    ...
    # Remove US country data from the global data df
    ...
    if(mode == 'Country/Region'){
        ...
        daily_data_us = read.csv("time_series_covid19_confirmed_US.csv")
    ...
    dot_color <- if(mode == 'Country/Region') "#D0104C"  else "#DDD23B"
    data_col <- which(colnames(daily_data) == converted_date_value)
    daily_data <- cbind(daily_data,daily_data[,data_col], log(200*(daily_data[,data_col])+1))

    colnames(daily_data)[(ncol(daily_data)-1):ncol(daily_data)] <- c('focal', 'logsize')
    ...
    plot_ly(
        ...
        type = 'scattermapbox',
        mode = 'markers',
        text = ~paste(Country.Region, '<br>', Province.State,'<br>Infection: ',focal)
    )
    fig <- fig %>%
    layout(
        mapbox = list(
